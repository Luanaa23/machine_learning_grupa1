...
np.random.seed(42)
num_samples = 100
education_year = np.random.randint(8, 20, size=(num_samples, 1)) # numarul de ani de ediucatie
experience_year = np.random.randint(0, 30, size=(num_samples, 1)) # numarul de ani de experienta
income = 2000 + 100 * education_year + 50 * experience_year + np.random.rand(num_samples, 1 ) * 500


# imaprtirea datelor in set de antrenare si set de test
...
ax[1].set_ylabel('Reziduri')
ax[1].set_title('Reziduri vs predictii')

plt.show()

print(f'Coeficientul de interceptare (beta): {model.intercept_}')
print(f'Coeficientul de inclinare (beta1, beta2): {model.coef_}')
print(f'Eroare medie patratica (MSE): {mse}')
